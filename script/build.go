...
var tasks = map[string]func(string) error{
	"bin/kubectl-role-diff": func(exe string) error {
		ldflags := os.Getenv("GO_LDFLAGS")
		ldflags = fmt.Sprintf("-X github.com/44smkn/kubectl-role-diff/pkg/build.Version=%s %s", version(), ldflags)
		ldflags = fmt.Sprintf("-X github.com/44smkn/kubectl-role-diff/pkg/build.Date=%s %s", date(), ldflags)
		return run("go", "build", "-trimpath", "-ldflags", ldflags, "-o", exe, "./cmd/kubectl-role-diff")
	},
...
func version() string {
	if versionEnv := os.Getenv("kubectl-role-diff_VERSION"); versionEnv != "" {
		return versionEnv
	}
